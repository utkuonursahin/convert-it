...
const ConvertButton = () => {
    ...
    const handleClick = async () => {
        const response = await fetch(`${window.location.href}api/convert?from=${currencyAtom.from}&to=${currencyAtom.to}&amount=${currencyAtom.amount}`, {cache: 'no-cache'})
        ...
        const historicalResponse = await fetch(`${window.location.href}api/historical?base=${currencyAtom.from}&start_date=${oneWeekAgo.toLocaleDateString('en-CA')}&end_date=${new Date().toLocaleDateString('en-CA')}&symbols=${currencyAtom.to}`, {cache: 'no-cache'})
